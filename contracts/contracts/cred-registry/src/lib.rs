...
mod cred_issuer_contract {
    soroban_sdk::contractimport!(
        file = "../../target/wasm32v1-none/release/cred_issuer.wasm"
    );
}
...
impl CredentialRegistry {
    ...
    pub fn init(env: Env, admin: Address, cred_issuer_contract: Address) {
        if env.storage().instance().has(&DataKey::Admin) {
            panic!("Already initialized");
        }
        env.storage().instance().set(&DataKey::Admin, &admin);
        env.storage().instance().set(&DataKey::CredIssuerContract, &cred_issuer_contract);
    }
    ...
    pub fn issue_credential(
        ...
        let certifier_id: Address = env.storage().instance().get(&DataKey::CredIssuerContract).expect("Not initialized");
        let certifier_client = cred_issuer_contract::Client::new(&env, &certifier_id);
