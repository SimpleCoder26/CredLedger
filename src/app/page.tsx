...
export default function Home() {
  ...
  useEffect(() => {
    const checkAuth = () => setIsConnected(!!localStorage.getItem("credledger_wallet"));
    checkAuth();
  ...
  return (
    <div className="bg-background text-on-background font-body-lg antialiased overflow-x-hidden selection:bg-signal-red selection:text-white bg-grid-pattern min-h-screen">
      ...
      <header className="fixed top-0 w-full z-50 bg-background/90 backdrop-blur-md border-b border-outline-variant/30 transition-all duration-300">
        <div className="max-w-[1280px] mx-auto px-margin h-16 flex justify-between items-center px-4 md:px-8 relative">
          ...
          <nav className="hidden md:flex absolute left-1/2 -translate-x-1/2 items-center gap-8">
            ...
            {isConnected && (
              <>
                <Link className="flex items-center gap-1 text-primary font-dot text-[16px] uppercase hover:text-pure-black transition-colors" href="/dashboard/issue"><LayoutDashboard className="w-4 h-4"/> Dashboard</Link>
                <Link className="flex items-center gap-1 text-on-surface-variant font-dot text-[16px] hover:text-pure-black transition-colors uppercase" href="/dashboard/history"><History className="w-4 h-4"/> History</Link>
                <Link className="flex items-center gap-1 text-on-surface-variant font-dot text-[16px] hover:text-pure-black transition-colors uppercase" href="/dashboard/settings"><User className="w-4 h-4"/> Profile</Link>
              </>
            )}
          </nav>
        ...
        {isMobileMenuOpen && (
          <div className="md:hidden absolute top-16 left-0 w-full bg-pure-white border-b border-outline-variant/30 shadow-lg p-6 flex flex-col gap-6">
            ...
            <div className="pt-4 border-t border-outline-variant/30 flex items-center justify-between">
              <span className="text-on-surface-variant font-dot text-[14px] uppercase tracking-wider">Theme Appearance</span>
              <ThemeToggle />
            </div>
          </div>
        )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-12 mb-16">
            <div className="md:col-span-2">
              <div className="flex items-center gap-3 mb-6">
                <span className="font-dot text-[32px] text-pure-black tracking-tight uppercase">CredLedger</span>
              </div>
