...
export default function IssuePage() {
  ...
  const { organizationName, defaultIssuerName } = useSettingsStore();
  const isConnected = !!address;
  const isProfileComplete = !!(organizationName && defaultIssuerName);
  ...
  const [manualSig1Name, setManualSig1Name] = useState("Alex Mercer");
  const [manualSig1Title, setManualSig1Title] = useState("COMMUNITY LEAD");
  const [manualSig2Name, setManualSig2Name] = useState("Jordan Lee");
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      // Point the QR code to our custom verification page
      const qrValue = typeof window !== 'undefined' 
        ? `${window.location.origin}/verify/${generatedCredentialId}`
        : `https://verify.credledger.com/verify/${generatedCredentialId}`;
                ...
                {manualIssueComplete ? (
                  <div className="pt-4 border-t border-primary">
                    ...
                    {blockchainTxHash && (
                      ...
                      >
                        <ExternalLink className="w-4 h-4" /> VERIFY ON PORTAL
                      </a>
                    )}
                    <button 
                      onClick={() => {
                  ...
                  <>
                    ...
                    <button 
                      ...
                      disabled={!isConnected || !isProfileComplete || txStatus === 'pending' || txStatus === 'processing' || !manualName}
